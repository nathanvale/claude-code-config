...
This tiered approach keeps CLAUDE.md lean while supporting unlimited scale in `memory/`.
...
- **Important pointer** -- to a durable note in `memory/` or `docs/`

Keep CLAUDE.md under ~150 lines. When a section grows large, extract the detail to `memory/` or `docs/` and leave a pointer.
...
2. memory/glossary.md        -> Full glossary if not in hot memory
...
| Type | CLAUDE.md (Hot Memory) | memory/ (Full Storage) |
|------|----------------------|------------------------|
...
| Preferences | Only if they affect most sessions in this repo | memory/context/ or related note |
