...
- **Interpreting an inbound comment: read the anchored line before deciding what it means.** Each comment's `inline.path` + `inline.line` point at a specific diff line. Use the configured Git-read MCP flow with JSON output, or the Bitbucket diff API when it exposes the hunk. Let the *changed code* tell you the topic. Do NOT infer the topic from the filename or from prior conversation: a comment on `checkbox.stories.tsx:1` is about whatever changed on line 1 (e.g. an import source), not whatever that file is "about". A reply that answers the wrong question costs a correction on the thread.
