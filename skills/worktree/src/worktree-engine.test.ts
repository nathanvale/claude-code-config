...
describe("drift detection", () => {
	...
	test("legacy wt-generated headers are accepted for drift checks", () => {
		const ws = renderWorkspace({ branches: {} }, twoWorktrees, () => true);
		const text = stampHeader(ws);
		const legacy = text.replace("GENERATED by worktree", "GENERATED by wt");
		expect(readHeaderHash(legacy)).not.toBeNull();
		expect(isDrift(legacy)).toBe(false);
	});
