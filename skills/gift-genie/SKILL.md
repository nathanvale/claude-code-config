...
- **Giver** — who am I being? (must have a profile in `memory/people/`)
...
Read **only**:
1. The giver's profile — `memory/people/<giver>.md`
2. The recipient's profile — `memory/people/<recipient>.md`
3. The recipient's product inventory — `memory/products/` (to dodge duplicates)
