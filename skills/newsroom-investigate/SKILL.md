...
description: Research any topic across Reddit, X, YouTube, and the web using the @side-quest/word-on-the-street CLI, then fact-check community claims against official sources. Dispatches parallel beat-reporter agents (one per topic) and a fact-checker agent. Use when the user asks "what are people saying about X", "best X recommendations", "what's the buzz around X", "X vs Y", "research X", "look into X", "find out about X", "has anyone tried X", "what's the latest on X", community sentiment, or any topic research query. Not for codebase exploration, code review, or implementation research.
