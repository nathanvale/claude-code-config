...
- **not achieved** — no effect is proven: the adapter rejected the action *before dispatch* (a stale ref that was never sent, as in PAC3/PAC4), or absence is proven at the persisted target. A rejection *after dispatch* does not prove no effect — the action may have committed before erroring; classify `unknown`. For a submit, save, or send, an unchanged page alone never proves no effect either — the mutation can commit server-side or land off-page.
