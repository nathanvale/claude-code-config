...
- [Selector-hallucination equalizer spike](research/2026-06-13-selector-hallucination-equalizer-spike.md) — proven: the fleet structurally rejects invented selectors (6/6 caught, model-independent Set-membership test) — moving selector-hallucination off the model and into code. The capability-equalizer angle: run a cheaper model safely instead of paying a frontier one to be careful.
