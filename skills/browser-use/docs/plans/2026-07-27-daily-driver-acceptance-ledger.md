...
treats "new device" as re-enrollment, not transfer); the paid Apple Developer
Program (US$99/yr) plus a Touch ID Magic Keyboard are not worth the marginal
security over a scoped, read-only service-account token for a personal box.
Accepted posture instead: `OP_SERVICE_ACCOUNT_TOKEN` injected at launch (never
persisted to `.zshrc`/`.env`/shell history), one read-only Browser Automation
vault as the damage boundary, rotate-on-move and revoke-on-theft as incident
response. Consequence for the ledger: the signed confidential-delivery lane is
the ONLY path that satisfies F06/F18 at tiers L/G, so under this decision those
tiers are WON'T-FIX (not "blocked pending operator") — C/H stay proven, L/G stay
permanently PARTIAL by choice. Same applies to any row whose only gap is the
