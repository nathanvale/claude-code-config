...
| --- | --- | --- |
| DDA-C01 | PASS | 2026-07-27 18:30 AEST / out-of-band stop: kill 7149 (SIGTERM to agent PID on 9223; no public stop surface — repair status only lists stores/leases, guide recovery constraint no_process_destruction), verified 9223 listener gone + no agent-warm-profile process; then WARM_CHROME_CDP_PORT=9231 browser-use task run --intent debug --tab 0 --allowed-origin http://127.0.0.1:8912 --json. stderr: warm-chrome check command-start port=9231 -> endpoint_unreachable -> cold launch -> new PID 52160 --remote-debugging-port=9231 --user-data-dir=/Users/nathanvale/.agent-warm-profile, lsof 52160 LISTEN 9231. Before: agent-warm-profile PIDs NONE. Exactly one launch. |
...
| DDA-C04 | PARTIAL | 2026-07-27 18:31 AEST / Gap: browser-use task run --help has no --profile/--environment flag; WARM_CHROME_PROFILE_DIR is warm-chrome-internal, not a front-door knob. Nearest variant: WARM_CHROME_CDP_PORT=9222 browser-use task run --intent debug --tab 0 --allowed-origin http://127.0.0.1:8912 --json (9222=human wrong profile) -> exit 20 code=foreign_listener, failure_class=foreign-listener, failure_domain=browser_entry_handoff, launch.launched=false, selected_lane absent (adapter_reached=false), constraint no_adapter_fallback. stderr: warm-chrome invalid_cdp -> browser-connect foreign_listener. Refused before adapter, but no public wrong-profile selector to exercise the criterion directly. |
...
| DDA-C06 | PARTIAL | 2026-07-27 18:32 AEST / agent-warm-profile held by PID 52160 (9231). WARM_CHROME_CDP_PORT=9233 browser-use task run --intent debug --tab 0 --allowed-origin http://127.0.0.1:8912 --json -> exit 20 code=launch_failed, failure_class=launch-failed, failure_domain=browser_entry_handoff, launch.launched=false, next_action_id=inspect_diagnostics, requires_operator=true, constraints[no_adapter_fallback,no_mutation_from_diagnostics], message 'Stop and inspect diagnostics; not a connection repair.'. stderr: check port=9233 -> endpoint_unreachable -> launch command-start -> spawned_unverified -> launch_failed. Attempts: launch=1, spawned_unverified=1 (no storm). Orphan check: no Chrome on 9233, only healthy 52160 remains. Gap: no public knob produces genuine remote-debugging-off on a live Chrome through the front door. |
| DDA-D01 | PARTIAL | 2026-07-27 18:43-18:44 AEST / WARM_CHROME_CDP_PORT=9231, agent-browser 0.31.2 on PATH / `browser-use task run --intent scrape --tab t1 --allowed-origin http://127.0.0.1:54159 --json` -> status ok, state confirmed, selected_lane agent-browser, lane_source intent-preferred, handoff_evidence_id ec65db89ecf9224862b01ac370140042 (auto-minted, no browser-connect), executed_steps 1, external_effect none. `browser-use targets list --mode recovery --adapter agent-browser --json` -> exit 20 target_discovery_transport_failed, next_action supply_verified_handoff 'Run browser-connect connect ... --handoff'. `browser-use targets list --mode handoff-bound --json` (no handoff) -> exit 20 target_discovery_handoff_invalid, same continuation. Missing intent: a front-door surface that emits the auto-minted handoff (or an auto-minting targets-list mode). |
| DDA-D06 | PARTIAL | 2026-07-27 18:46 AEST / fixture http://127.0.0.1:54159/observe / observe#1 (PENDING/pre-node): `task run --intent scrape --tab t2 --json` -> status ok, state confirmed, handoff 77d56ffcfb177f3d4503d3abce3e219c, steps 1. curl /mutate flipped server DOM to DONE/post-node/data-counter=1. observe#2 (DONE/post-node): -> status ok, state confirmed, FRESH handoff 673135b32973b502e8c4113d3ac69ecb, steps 1, artifacts []. `artifact list --json` -> artifact_count 0. Source: browser-use.ts baselineAgentBrowserTask = single snapshot; no mutate intent in `task list` (routine-automation/runbook-execution require an authored runbook; sole shipped runbook is read-only + auth-bound). Missing: front-door page-mutation surface + observed-structure exposure. |
...
| DDA-D09 | PARTIAL | 2026-07-27 18:46 AEST / agent-browser lane live / Each `task run --intent scrape` mints a distinct handoff (observe#1 77d56ffc vs observe#2 673135b3) and executes exactly steps:1 snapshot — proven fresh-observation-per-run, no cross-run ref carry. Source browser-use-agent-browser.ts:778 'A fresh task-local snapshot is required immediately before ref mutation'; :655 'discards refs after every mutation, verifies fresh structure before reporting confirmed'. `task run --help` exposes no ref/snapshot/carry flag. Refusal machinery real but unreachable: needs a mutating multi-step task the front door does not expose. |
...
| DDA-D12 | PARTIAL | 2026-07-27 18:47 AEST / fixture /ambiguous (undetectable-effect page) / `browser-use task run --intent scrape --tab t3 --allowed-origin http://127.0.0.1:54159 --json` -> status ok, state confirmed, executed_steps 1, external_effect none. Read-only baseline dispatches no ambiguous effect; guide --full documents the confirmed/not-achieved/unknown doctrine but the front door has no intent that mutates, so `unknown` is unreachable via `task run`. Missing intent: an externally-effecting mutation the front door would classify. |
| DDA-D13 | PARTIAL | 2026-07-27 18:47 AEST / fixture /liar (banner role=status 'Saved! ... saved successfully' + div data-persisted=false) / `browser-use task run --intent scrape --tab t2 --allowed-origin http://127.0.0.1:54159 --json` -> status ok, state confirmed, executed_steps 1, external_effect none. baselineAgentBrowserTask carries no postcondition; the chrome-devtools variant is additionally pin-blocked (1.2.0 vs 1.5.0). Missing: a front-door mutating intent with a named structural postcondition to override. |
