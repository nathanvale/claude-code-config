...
| DDA-D06 | PARTIAL | 2026-07-27 / hermetic front-door proof: `routine-automation` resolves exactly one current semantic target, durably records mutation dispatch, clicks once, then confirms only from a fresh named structural postcondition. Zero/multiple semantic matches, stale semantic reassignment, origin drift, and whitespace selectors refuse before mutation. Live rerun with `WARM_CHROME_CDP_PORT=9231` blocked before browser launch because the pinned `agent-browser` adapter is not installed; no pin-policy change authorized. |
...
| DDA-D09 | PARTIAL | 2026-07-27 / hermetic front-door proof: public `routine-automation` resolves semantics from the current task-local snapshot, requires exactly one match, and refuses a ref whose current semantics differ before dispatch. Mutation clears snapshot refs and verifies from fresh structure. Live rerun blocked by the absent pinned `agent-browser` adapter. |
...
| DDA-D12 | PARTIAL | 2026-07-27 / hermetic front-door proof: verification loss after a dispatched semantic click records `state: unknown` and `external_effect: unknown`; retry returns the existing terminal run and issues no second click. The pre-dispatch mutation marker is persisted through CAS, including the crash window. Live rerun blocked by the absent pinned adapter. |
| DDA-D13 | PARTIAL | 2026-07-27 / hermetic front-door proof: ambient success text cannot override a false named structural postcondition; the run records not-achieved with unknown external effect after the single dispatch. Live rerun blocked by the absent pinned adapter. |
