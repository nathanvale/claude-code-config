...
## P0 Safety Receipts (2026-07-28)

| Row | Verdict | Receipt |
| --- | --- | --- |
| DDA-D08 | BLOCKED | Mutation implementation complete: `src/browser-use-playwright-task.test.ts` proves fresh snapshot ref resolution, write-ahead mutation marking, one click, fresh visible postcondition, unknown-on-click/verification loss, and false-postcondition refusal. `src/browser-use-task-run.test.ts` proves the same through `browser-use task run`; `src/browser-use-platform-process-boundary.test.ts` spawns the real CLI from a neutral CWD through a fake absolute Playwright executable and gets a confirmed shared run. Full `skills/browser-use/src` suite passes. Required tier-L receipt blocked: `browser-connect repair-adapter playwright-cdp --check --json` run `edded46a-deb2-4f6c-acd7-3936eb8e064c` reports install_state absent, pinned `@playwright/cli` 0.1.17, lifecycle_scripts_required, requires_operator, and `no_pin_policy_change`. |
| DDA-D11 | BLOCKED | `src/browser-use-playwright-task.test.ts` proves zero/duplicate current semantic matches refuse before the write-ahead marker and issue no click. `src/browser-use-task-run.test.ts` proves a stale semantic ref records `playwright_task_ref_invalid`, `external_effect=none`, `mutation_dispatched=false`, and never confirms; click uncertainty records terminal unknown and exactly one click. Required tier-L stale-locator receipt shares the D08 operator install gate above. |
| DDA-F06 | PARTIAL | `src/browser-use-confidential-field-delivery-leak.test.ts` (3 tests / 29 expect) sweeps password/OTP success, helper-crash, and negative leak fixtures; `src/browser-use-confidential-delivery-seam.test.ts` (12 tests / 100 expect, +1 wired H-tier over 11/79) sweeps real on-disk run bytes and failed-after-delivery outcomes; `src/browser-use-confidential-delivery-interruption.test.ts` (1 test / 6 expect) kills a real child after its helper observes a derived password sentinel and proves stdout, stderr, runtime/state roots, and fixture artifacts contain zero sentinel bytes. Wired-seam runbook-delivery coverage added 2026-07-28: `src/browser-use-runbook.test.ts` (38 tests / 104 expect, +5 C-tier over 33) drives the confidential path through the real runbook executor (`src/browser-use-runbook.ts` / `src/browser-use-runtime.ts`, wiring proven by `src/browser-use-runtime-security-wiring.test.ts`), backed by `src/fixtures/confidential-runbook-delivery-fixture.ts` (standalone smoke exit 0, journal [quarantine:raised, lease:acquired:oncore_password, op-execute:secret-acquired, delivery:bounded-write, cleanup:released], run.json sentinel count 0). Full `skills/browser-use/src` suite 63 files / 1298 tests / 7676 expect / exit 0; tsc_check errorCount 0; biome clean. C/H strengthened. Public-front-door receipt 2026-07-28, run `1a6aa68d-825e-47ad-82af-0252697d2a6e`: `browser-use auth enroll-browser-automation-token --json` returned `native-capability-absent`, continuation `acquire-native-capability`, because the signed Browser Use Security product does not exist. Tier L/G stay PARTIAL behind the native-product-absent operator gate (Apple signing U04, 1Password U05, Keychain enrollment U06); `runtime/browser-use-security/` now scaffolds the admission manifest + unsigned targets but is not admissible until signed. A public live auth workflow cannot begin. |
| DDA-F18 | PARTIAL | Process-boundary kill receipt: `src/browser-use-confidential-delivery-interruption.test.ts` (1 test / 6 expect) launches the real confidential-delivery choreography in a child (`src/fixtures/confidential-delivery-interruption-fixture.ts`), waits until the helper has observed a derived sentinel, sends SIGKILL, then scans every file plus stdout/stderr. Test passes; no cleanup assumption is required. Wired-seam extension 2026-07-28: `src/browser-use-runbook.test.ts` (+5 C-tier, 38/104) plus `src/fixtures/confidential-runbook-delivery-fixture.ts` prove the real runbook executor's bounded-write delivery + cleanup:released path leaves run.json sentinel count 0. The same public auth receipt (`1a6aa68d-825e-47ad-82af-0252697d2a6e`) proves tier-L interruption is unreachable before secret acquisition: native custody is absent and the public continuation requires the unshipped signed Browser Use Security product (native-product-absent operator gate: Apple signing U04, 1Password U05, Keychain enrollment U06). |
| DDA-F26 | PASS | `runtime/warm-chrome/tests/default-profile-safety.test.ts` passes five contract/hermetic cases: forbidden persistent-setting tokens across shipping `browser-use`, `browser-connect`, and `warm-chrome` source; AST allowlist of every Warm Chrome file-content mutation; adversarial split/dynamic/aliased write fixtures; legitimate read control; default-vs-dedicated profile matcher. Existing `check-stations.test.ts`, `launch-stations.test.ts`, `repair-stations.test.ts`, and `runtime.test.ts` cover attach, verify, launch, repair, symlink, and resolved-path refusal. Full `runtime/warm-chrome/tests` suite passes. Remote debugging remains transient `--remote-debugging-port` launch state only; no shipping path writes Chrome's persistent setting. |

## Environment-token hermetic graduation (2026-07-30)

| Unit | Verdict | Receipt |
| --- | --- | --- |
| U9 | PASS (hermetic composite) | `src/browser-use-packaged-install.integration.test.ts` runs a real build and `bun pm pack`, installs the tarball into an empty consumer, proves all three native helpers are executable and byte-identical to the release artifacts inside `dist/bin`, then runs installed `browser-use` help, runbook discovery, and missing-token status from a neutral CWD with a clean HOME/XDG environment and no ambient token. The first red run exposed fresh-home status reporting `unsafe`; production now admits and canonicalizes only the private config root before invoking native custody, so the installed command returns typed `missing-token` plus `install-local-token`, including an operator-owned XDG symlink layout. The installed bundle's programmatic public parser/runtime contract separately admits earned fake metadata/support evidence and reports the hermetic lower-assurance lane ready with zero credential-field retrieval. This is deliberately compositional: shipping code never gains a fake-OP switch or token-custody bypass. The native Swift suite drives the byte-identical custody, OP supervisor, confidential-delivery helper, fake OP, websocket fixture, crash/cancel/timeout/replay, and sentinel-leak boundaries. Source conformance, auth-command, runtime-security, quarantine, interruption, and leak suites retain the failure matrix. This receipt proves the no-network/no-live-secret U9 gate only. Real admin authority, effective running-Chrome posture, route binding, Oncore authentication, and portal completion remain U10 live evidence. |
...
| DDA-D01 | PASS | 2026-07-28 AEST / `WARM_CHROME_CDP_PORT=9231 browser-use targets list --mode handoff-bound --adapter agent-browser --json` auto-minted a handoff (no browser-connect) and emitted operation-ready candidates: status ok, operation_ready true, candidate_count 1, candidate ordinal 1 origin `http://127.0.0.1:8912` (R32-redacted), run_id `dcbf5e75-78fa-4b31-8596-9992eaf48a58`, handoff_evidence_id `6092f7a42dd638eba6acaa4e0de8810a`, `361ms`, exit 0. The agent-browser lane gained its own CLI-subcommand discovery transport (`<probe> --cdp <ws> --session browser-use-<run> tab list --json`); gate stays fail-closed (playwright-cdp refuses, chrome unchanged). |
...
| DDA-D26 | PASS | 2026-07-28 AEST. Reduced Agent Chrome to zero page targets via out-of-band CDP close (json/close cascade); CDP still alive on 9231, page targets=0. `WARM_CHROME_CDP_PORT=9231 browser-use targets list --mode handoff-bound --adapter agent-browser --json` -> exit 20, the exact named oracle `target_discovery_no_candidates` ("No Browser Target Candidates were discovered through the attached adapter."), continuation `open_browser_target`, run_id `757e03b4-1a6c-480e-80af-7a555f8e67d1`, `520ms`, stderr no stack trace = never a crash. The agent-browser CLI-subcommand discovery transport now reaches this continuation live. Agent Chrome restored to a fixture tab; post-restore discovery green (candidate_count 1). |
| DDA-D27 | PASS | 2026-07-28 AEST. Opened extra fixture tabs via out-of-band CDP against 9231 -> 7 total page targets (loopback fixture on 127.0.0.1:8912). `WARM_CHROME_CDP_PORT=9231 browser-use targets list --mode handoff-bound --adapter agent-browser --show-url --json` stayed bounded and typed: status ok, operation_ready true, candidate_count 7, dense ordinals [1..7], R32 redaction intact (each origin `http://127.0.0.1:8912`, path_shape `/ […]`, query stripped), handoff_evidence_id `cbea1f32311dddc8fb162715615ee69f`, `285ms`, exit 0. The candidate-list/ordinal/redaction surface now runs live through the agent-browser CLI-subcommand discovery transport. (`--url-contains` is a `targets select` flag, not `targets list`; listing exposes `--show-url`.) Fixture tabs closed afterward, restored to one. |
