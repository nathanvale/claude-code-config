#!/usr/bin/env bun

import { mkdir, readFile, writeFile } from "node:fs/promises";
import { isAbsolute, join, resolve } from "node:path";
import { parseArgs } from "node:util";
import { loadIdentityCandidates } from "../../people-enrich/scripts/people-note.ts";
import {
  buildProfileBundleTarget,
  renderBundle,
  resolveTargetCandidate,
  runClaude,
  type BundleTarget,
} from "./voice-enrich.ts";

type ReplyArtifactPaths = {
  bundlePath: string;
  replyPath: string;
};

type ReplyEvidence = {
  kind: "text-message" | "thread-summary";
  path: string;
  content: string;
};

const MY_SECOND_BRAIN_ROOT = "/Users/nathanvale/code/my-second-brain";
const CLAUDE_CONFIG_ROOT = "/Users/nathanvale/code/claude-code-config";
const PEOPLE_DIR = join(MY_SECOND_BRAIN_ROOT, "memory/people");
const RUNTIME_TMP_DIR = join(
  MY_SECOND_BRAIN_ROOT,
  "runtime/people-enrichment/tmp",
);
const NATHAN_PROFILE_PATH = join(PEOPLE_DIR, "nathan-vale.md");
const OUTPUT_CONTRACT_PATH = join(
  CLAUDE_CONFIG_ROOT,
  "context/contract-text-message.md",
);

async function main(): Promise<void> {
  const parsed = parseArgs({
    args: Bun.argv.slice(2),
    options: {
      "message-file": { type: "string" },
      "thread-summary": { type: "string" },
    },
    allowPositionals: true,
    strict: true,
  });

  const requestedName = parsed.positionals.join(" ").trim();
  if (!requestedName) {
    throw new Error(
      "Usage: bun run voice-text-reply.ts <name> --message-file /abs/path.md [--thread-summary /abs/path.md]",
    );
  }

  const messageFile = normalizeAbsolutePath(
    parsed.values["message-file"],
    "--message-file",
  );
  const threadSummaryPath = normalizeOptionalAbsolutePath(
    parsed.values["thread-summary"],
    "--thread-summary",
  );

  await mkdir(RUNTIME_TMP_DIR, { recursive: true });

  const target = await resolveTarget(requestedName);
  const [nathanProfileContent, outputContractContent, incomingMessage] =
    await Promise.all([
      readFile(NATHAN_PROFILE_PATH, "utf-8"),
      readFile(OUTPUT_CONTRACT_PATH, "utf-8"),
      readFile(messageFile, "utf-8"),
    ]);

  const evidence: ReplyEvidence[] = [
    {
      kind: "text-message",
      path: messageFile,
      content: incomingMessage,
    },
  ];

  if (threadSummaryPath) {
    evidence.push({
      kind: "thread-summary",
      path: threadSummaryPath,
      content: await readFile(threadSummaryPath, "utf-8"),
    });
  }

  const artifactPaths = buildArtifactPaths(target.candidate.slug);
  const bundle = renderBundle({
    mode: "review",
    taskBrief: buildTaskBrief(target.target.name),
    nathanProfileContent,
    target: target.target,
    guidance: {
      confidence: "full",
      recommendProfileCreation: false,
    },
    evidence: evidence.map((item) => ({
      ...item,
      format: "markdown" as const,
    })),
    outputContractPath: OUTPUT_CONTRACT_PATH,
    outputContractContent,
  });
  await writeFile(artifactPaths.bundlePath, bundle, "utf-8");

  const dispatch = await runClaude({
    prompt: buildReplyPrompt(bundle, artifactPaths.bundlePath),
  });
  const reply = dispatch.stdout.trim();
  assertTextReply(reply);

  await writeFile(artifactPaths.replyPath, `${reply}\n`, "utf-8");

  process.stdout.write(
    `${JSON.stringify(
      {
        workflow: "text-reply",
        note_path: target.notePath,
        bundle_path: artifactPaths.bundlePath,
        reply_path: artifactPaths.replyPath,
      },
      null,
      2,
    )}\n`,
  );
}

type ResolvedReplyTarget = {
  notePath: string;
  target: BundleTarget;
  candidate: { slug: string };
};

async function resolveTarget(requestedName: string): Promise<ResolvedReplyTarget> {
  const candidates = await loadIdentityCandidates(PEOPLE_DIR);
  const candidate = resolveTargetCandidate(requestedName, candidates);
  if (!candidate.filePath) {
    throw new Error(`Resolved ${candidate.title} without a backing file path.`);
  }

  const notePath = resolve(candidate.filePath);
  const noteContent = await readFile(notePath, "utf-8");
  return {
    notePath,
    target: buildProfileBundleTarget({
      candidate,
      notePath,
      noteContent,
    }),
    candidate: { slug: candidate.slug },
  };
}

function normalizeAbsolutePath(input: string | undefined, flagName: string): string {
  if (!input) {
    throw new Error(`${flagName} is required.`);
  }
  if (!isAbsolute(input)) {
    throw new Error(`${flagName} must be an absolute path.`);
  }
  return resolve(input);
}

function normalizeOptionalAbsolutePath(
  input: string | undefined,
  flagName: string,
): string | undefined {
  if (!input) return undefined;
  if (!isAbsolute(input)) {
    throw new Error(`${flagName} must be an absolute path.`);
  }
  return resolve(input);
}

export function buildArtifactPaths(targetSlug: string): ReplyArtifactPaths {
  return {
    bundlePath: join(RUNTIME_TMP_DIR, `${targetSlug}-text-reply-bundle.md`),
    replyPath: join(RUNTIME_TMP_DIR, `${targetSlug}-text-reply.txt`),
  };
}

export function buildTaskBrief(targetName: string): string {
  return [
    "Mode: reply",
    `Task: Draft a text reply to ${targetName} using only the supplied context.`,
    "Return plain text only.",
    "If text is the wrong medium for the moment, draft a short bridge text that moves toward a call or in-person conversation without litigating the issue by text.",
  ].join("\n");
}

export function buildReplyPrompt(bundle: string, bundlePath: string): string {
  return [
    `The ContextBundle at ${bundlePath} has already been assembled in deterministic order.`,
    `The output contract is ${OUTPUT_CONTRACT_PATH}.`,
    "Do not read any files or widen scope. Treat the bundle below as the entire allowed context.",
    "Return only the sendable text message in plain text. Do not emit markdown, bullets, or JSON.",
    "",
    bundle,
  ].join("\n");
}

export function assertTextReply(reply: string): void {
  if (!reply.trim()) {
    throw new Error("Text reply output is empty.");
  }
  if (reply.includes("```")) {
    throw new Error("Text reply output must not include code fences.");
  }
  if (/^#|\n#|^\* |\n\* |^- |\n- /m.test(reply)) {
    throw new Error("Text reply output must not include markdown headings or list formatting.");
  }
}

if (import.meta.main) {
  await main();
}
