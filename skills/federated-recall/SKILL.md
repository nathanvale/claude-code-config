...
3. `~/.config/memory/docs/qmd-federation.md`
4. `~/.config/memory/docs/memory-recall-examples.md`
5. `~/.config/memory/docs/qmd-query-mode-decision.md`
...
2. If QMD MCP is unavailable, use the local QMD wrappers:
   - `~/.config/memory/scripts/qmd-recall.sh "query"` for lightweight default recall
   - `~/.config/memory/scripts/qmd-recall.sh --rich "query"` only as an explicit opt-in
   - `~/.config/memory/scripts/qmd-node.sh get ...` for opening a chosen result
