...
Prepare a new sprint. Writes a sprint project doc under `context/projects/sprint-NN.md` and rewrites `TASKS.md` so it leads with current work, not stale history. Verifies sprint naming against the live Jira board, surfaces recent planning and standup signal, and asks for missing context before writing.
...
- A prior sprint doc at `context/projects/sprint-*.md` to use as template — OR run with `--bootstrap` to create one from scratch
...
- A **template** sprint doc exists at `context/projects/sprint-*.md` that is **not the target sprint** itself → if not and no `--bootstrap` flag, **stop** and offer: "no prior sprint doc to use as template — re-run with `--bootstrap` to create one from scratch"
...
```bash
git status --short TASKS.md context/projects/sprint-*.md
```
...
```bash
ls context/projects/sprint-*.md | grep -v "sprint-<TARGET>.md"
```
...
Read all `feedback_*.md` under `~/.claude/projects/<project-slug>/context/` — not just `feedback_sprint*`. Important rules live under names like `feedback_cypress_dedicated_ticket.md`, `feedback_devops_template_escalation.md`, etc.
...
**Action:** append to `context/sprint-history.md` (newest-first). Create with frontmatter `title: Sprint History / type: archive / status: rolling / updated: <today>` if missing.
...
2. Write/append to `context/sprint-history.md`: insert after `# Sprint History\n` line; create file with frontmatter if missing.
...
5. Bump `context/sprint-history.md` `updated:` frontmatter.
...
If `context/projects/contract-extension.md` (or equivalent) exists, read it for the contract end date. Run the 6-step parse chain (ISO → long-form → fuzzy month-only → quarter → relative weeks → unparseable) — full recipe: [`references/parsing-and-buckets.md`](references/parsing-and-buckets.md) § "Contract date parsing chain".
...
**Idempotency guard** — if `context/projects/sprint-NN.md` already exists, diff before overwriting:
...
**Then write** `context/projects/sprint-NN.md` — frontmatter + sections matching the previous sprint's template, populated with:
...
Project doc: context/projects/sprint-NN.md (NEW or UPDATED)
...
6. Brainstormed prep:
   - Archive Sprint 22 done table to `context/sprint-history.md`
...
8. Write `context/projects/sprint-24.md` + rewrite TASKS.md.
9. Report.
...
- `context/feedback_sprint*` — repeat sprint directives the skill reads at Step 5
- `context/feedback_use_raw_notion_transcripts.md` — Notion AI summary trap
- `context/feedback_verify_quote_speaker_with_nathan.md` — speaker attribution guard
- `context/feedback_cypress_dedicated_ticket.md` — don't fold tech-debt into feature scope
