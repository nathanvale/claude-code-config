...
/**
 * Load identity candidates from the existing `context/people` directory.
 */
...
export function buildPersonPatch(input: BuildPersonPatchInput): PersonPatch {
  ...
  const mergedHandles = mergeSourceHandles(existingHandles, {
  ...
  if (Object.keys(mergedHandles).length > 0) {
    mergedFrontmatter.source_handles = mergedHandles;
  } else {
    delete mergedFrontmatter.source_handles;
  }
