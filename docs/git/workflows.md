...
1. **Verify on feature branch**: `git branch --show-current` -- abort if on a protected branch
2. **Find merge base**: `git merge-base <default-branch> HEAD`
3. **Review commits**: `git log --oneline $(git merge-base <default-branch> HEAD)..HEAD`
4. **Review total diff**: `git diff $(git merge-base <default-branch> HEAD)..HEAD --stat`
5. **Soft reset**: `git reset --soft $(git merge-base <default-branch> HEAD)`
6. **Create single conventional commit** -- follow the Commit workflow above
7. **Verify**: `git log --oneline -3` to confirm clean history

**Safety**: Never squash on a protected branch. Always verify branch first.
...
1. **Verify all changes committed**: `git status --porcelain -b`
2. **Check current branch**: `git branch --show-current`
3. **Review all commits**: `git log --oneline <default-branch>..HEAD`
...
5. **Review total diff**: `git diff <default-branch>...HEAD --stat`
...
**Key rules:**
- Draft -> Show -> Approve -> Post (default flow)
...
1. **Determine branches**: Current branch vs `[branch]` argument (default: default branch)
...
If no validate script exists, report "No validate script found -- skipping validation gate." and proceed.
...
**Failure handling:**
- Non-fast-forward: "Remote has N commits you don't have. Resolve manually: 1) `git pull --rebase` 2) `git pull` (merge) 3) Abort."
...
Run in 3 sequential rounds (dependency chain):
...
git for-each-ref --format='%(refname:short)%00%(objectname:short)%00%(creatordate:relative)%00%(upstream:track)%00%(subject)%00' -- refs/heads/  # NUL-delimited
```
...
Show categorized results with metadata:
...
If `--confirm` was NOT passed, stop here.
...
3. **Handle `-d` failures (squash merge detection):**
   ...
   Detect default branch, then run tree-comparison squash merge detection:
   ```bash
