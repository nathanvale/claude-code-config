---
date: 2026-06-02
topic: create-cli-result-vocabulary-guidance
title: "create-cli guidance for package-owned result vocabulary"
type: brainstorm
issue: 155
---

# create-cli guidance for package-owned result vocabulary

## Summary

Update create-cli docs so agents catch package-owned result vocabulary while respecting facade
...
PR #76 moved several facade candidates into runtime-backed enforcement. Existing create-cli guidance
...
- **Docs compose.** `skills/create-cli/SKILL.md` routes. Existing refs teach. No new parallel
...
- R1. Update create-cli wording for PR #76 runtime-backed affordances: baseline exits, diagnostic
...
- R7. Guidance must avoid listing package-specific literal values in create-cli prose.
...
- R11. `skills/create-cli/SKILL.md` may route to references, but must not duplicate the facade
  reference body.
- R12. Examples must teach judgment, not full schemas, allowed-value lists, or runtime envelopes.

## Scope Boundaries

- In scope: update `skills/create-cli/references/agent-native-cli-design.md`.
- In scope: update `skills/create-cli/references/cli-command-facade.md`.
...
- Out of scope: listing Browser Adapter Proof statuses, source labels, diagnostic codes, or result
  members in create-cli prose.
- Out of scope: new reference file unless existing refs become materially noisy.

## Acceptance Examples

- AE1. **Covers R1-R3.** Given create-cli docs mention baseline exits, when an agent reads them after
...
- AE2. **Covers R4-R7.** Given a package has stable `data.*` status strings, source labels, or
  diagnostic labels, when create-cli guidance covers result contracts, then it directs the agent to
...
- `rg "does NOT judge sensible exit codes|contract candidate" skills/create-cli/references CONTEXT.md`
