...
The current `wt` skill is the right workflow entry point for day-to-day worktree and VS Code workspace workflows, but it still delegates git/worktree truth to `@side-quest/git`. That leaves the agent-native recovery contract split across a skill, an external CLI, and local glue.
...
- **Shared inline runtime package over external dependency.** Rebuild the SideQuest Git subset inside this repo as a shared workspace package so `wt` and future skills consume one local owner for git, worktree, hygiene, and recovery behavior.
...
- **`wt` remains the workflow entry point.** The skill should route humans and agents into the new package instead of growing its own git/worktree logic.
...
- A3. **Workflow entry point:** `wt` and later skills call the shared package through stable command contracts.
...
- F3. **Use `wt` through the new owner**
  - **Trigger:** A human or agent asks to create, remove, clean, render, or open worktrees through `wt`.
  - **Actors:** A1, A2, A3, A4.
  - **Steps:** `wt` calls the shared package for git/worktree truth, then keeps owning workspace rendering and VS Code ergonomics.
...
- R8. The SideQuest Git rebuild must live in this repo as a shared workspace package consumed by `wt` and future skills.
...
- R14. `wt` must keep owning VS Code workspace rendering, focus/color preferences, drift gates, and open behavior.
- R15. `wt` must stop depending on `@side-quest/git` once the shared package exposes equivalent worktree lifecycle behavior.
- R16. `wt` must report shared-package failures through its own facade envelope without losing the upstream recovery classification.
...
- AE5. **Covers R14, R15, R16.** Given `wt clean` calls the shared package and the shared package blocks on dirty state, when `wt` returns, then its envelope preserves the shared failure category and points to the safe inspection path.
...
- `wt` can create, remove, clean, and render worktrees without shelling to `@side-quest/git`.
...
- Config sync and install hooks beyond the minimum needed for `wt` migration.
...
- `wt` is the first consumer and proof target for the shared package.
- The accepted package identity is `agent-worktree`, with `agent-worktree` as the canonical command and `awt` as the alias.
...
- Migration sequence for replacing `@side-quest/git` in `wt`.
