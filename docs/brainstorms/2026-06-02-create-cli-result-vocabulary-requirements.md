---
date: 2026-06-02
topic: cli-author-result-vocabulary
---

# CLI Author Result Vocabulary Requirements

## Summary

Add a small cli-author guidance update that makes `skills/cli-author/references/cli-command-facade.md` the canonical owner for package-owned result vocabulary guidance. The update should clarify that the facade owns shared contract shape, while each package owns stable agent-facing literals inside that shape.

---

## Problem Frame

The Browser Adapter Proof work exposed a cli-author guidance gap. `@side-quest/cli-command-facade` makes shared CLI contract shape explicit, but stable package-specific output values can still become agent-facing contract without being owned by the facade schema.
...
- **Canonical owner:** Put the guidance in `skills/cli-author/references/cli-command-facade.md`, near `resultContract` and runtime output guidance.
- **Reference-only update:** Leave `skills/cli-author/SKILL.md` unchanged; it already routes implementers to the facade reference.
...
- No update to `skills/cli-author/SKILL.md`.
- No broad rewrite of cli-author guidance.
...
- AE2. **Covers R7-R8.** Given package-specific values already exist in one package, when cli-author documents the principle, then it names the ownership rule and example categories without copying that package's full value catalog.
...
- Current cli-author front door: `skills/cli-author/SKILL.md`
