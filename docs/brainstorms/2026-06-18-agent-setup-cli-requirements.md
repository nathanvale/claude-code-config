...
Replace `install.sh` with `setup`, a single agent-native CLI that owns symlink topology, git hook installation, and agent-instructions health reporting. Skills are excluded (managed by `npx skills`). The CLI provides ADHD-friendly observability, repair hints, and structured JSON output for agents.
...
- **Full install.sh replacement.** One tool owns everything install.sh does. No coexistence. install.sh is deleted after setup is proven.
- **Named `setup`.** Lives at `runtime/setup`.
...
- **Skills excluded.** Managed by `npx skills`. setup does not touch `~/.claude/skills/`, `~/.codex/skills/`, or `.agents/skills/`.
...
- R1. Bare command (`setup`) adapts output based on state: calm dashboard, doctor, or clean-slate summary.
- R2. `setup sync` creates missing links, fixes wrong/broken links, installs git hooks, runs agent-instructions health check. Stops on conflicts without writing.
- R3. `setup sync --check` previews what sync would do. Exit 0 when clean, 1 when sync needed.
- R4. `setup unlink` removes all managed symlinks.
- R5. `setup help` provides progressive ADHD-friendly help: what the tool does, what it manages, what it doesn't, typical workflow.
...
- R11. `settings.local.json` is machine-specific and stays gitignored. setup does not touch it.
...
- R21. The TypeScript CLI lives at `runtime/setup/src/cli.ts`.
...
- AE1. Given a fresh clone with no existing links, `setup` shows a compact clean-slate summary with one next action.
- AE2. Given `setup sync` on a fresh clone, all 13 links are created, git hooks installed, agent-instructions check runs, and the command exits 0.
- AE3. Given one conflict (real file at `~/.claude/settings.json`), `setup sync` creates the other 12 links but stops on the conflict with a repair hint.
- AE4. Given a healthy state, `setup` shows a calm dashboard with "All good. Nothing to do."
- AE5. Given a wrong link, `setup` shows doctor output with "why it matters" and a copy-pasteable fix command.
- AE6. Given `setup sync --check --json`, the output is a valid agent-native envelope with `next_safe_action` and `changed_state: none`.
- AE7. Given `setup` on a machine without Bun, the shell entrypoint offers to install Bun before proceeding.
