...
- Browser Use probes token presence without copying its value into TypeScript runtime state. The versioned exact-environment wrapper is the sole designated retrieval child: after opening and validating the token file, it constructs the exact environment and immediately `exec`s the official `op` command. No wrapper-spawned intermediate child exists; the parent process boundary remains an explicit lower-assurance exposure.
- Every Browser Use child environment excludes `OP_SERVICE_ACCOUNT_TOKEN` except that single wrapper-to-`exec(op)` retrieval process. Adapter, browser, helper, setup, and diagnostic children never inherit it.
