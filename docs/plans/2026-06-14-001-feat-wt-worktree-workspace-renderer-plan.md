---
title: "feat: wt — worktree workspace renderer CLI + thin skill"
type: feat
date: 2026-06-14
origin: docs/brainstorms/2026-06-14-wt-worktree-workspace-renderer-requirements.md
---

# feat: `wt` — Worktree Workspace Renderer CLI + Thin Skill

## Summary

Build `wt`, a facade-backed agent-native CLI (under `skills/wt/`) that renders a per-repo VS Code `.code-workspace` from a branch-keyed registry joined with live worktree state, baking in ADHD scaffolding (per-worktree color, window title, focus-folder pairs, noise excludes, WIP folder). Owned verbs (`sync`/`focus`/`color`/`open`) render; delegated verbs (`new`/`rm`/`clean`) shell out to `@side-quest/git worktree` and re-render. A thin `SKILL.md` names owners and routes; the `.code-workspace` becomes generated output, drift-gated against manual edits.

---

## Problem Frame

Nathan hand-edits his VS Code workspace file across git worktrees. Manual editing is fragile, doesn't scale across repos, and carries no ADHD scaffolding (identical window names/colors/noisy trees → wrong-window commits, "which window was I in?" tax). The fix: make the workspace generated from a durable source of truth, with color/naming/focus/noise-reduction baked into the render. See origin: `docs/brainstorms/2026-06-14-wt-worktree-workspace-renderer-requirements.md`.
...
- R1. `wt sync [repo]` renders `<repo>.code-workspace` from the registry joined with live `@side-quest/git worktree list` output.
...
- R7. Prefs live in a gitignored `wt.config.json` at repo root, keyed by branch name, so prefs survive worktree delete/recreate.
- R8. `wt focus <branch> <subfolder>` and `wt color <branch> <color>` write the registry, then re-render.

### Delegation (one front door)

- R9. `wt new <branch>` / `wt rm <branch>` / `wt clean` delegate to `@side-quest/git worktree` (`create` / `delete` / `orphans --delete`), then re-render.
- R10. Delegated verbs never re-implement worktree CRUD; they parse the upstream CLI's JSON output.

### Launcher

- R11. `wt open [name]` opens the named repo's workspace via `code`; with no arg, it lists known workspaces to stdout (parseable).

### Drift safety

- R12. The rendered `.code-workspace` carries a `GENERATED by wt` header plus a content hash.
- R13. `wt sync` detects when the file changed since the last render and blocks overwrite: show a diff on stderr, require `--force` (or interactive confirm); non-interactive runs get a distinct drift-blocked exit code plus a structured action. Never silently clobbers.
...
- **Facade-backed lane, mirroring `record-decision`**: `wt` carries the same `src/command-contract.ts` + `src/model.ts` + `src/wt.ts` (dispatcher) + `src/wt-engine.ts` (pure render) + `src/wt.test.ts` layout and depends on `@side-quest/cli-command-facade` (`workspace:*`). Rationale: the repo already runs this lane; the facade enforces the four drift surfaces R14 needs; `record-decision` is the closest analog (small, write-oriented, owned + dry-run verbs). (see origin: CLI contract table)
- **Pure render engine**: `wt-engine.ts` is `(registry, worktreeList, defaults) → workspaceJSON` with no I/O — testable in isolation, the heart of the Command Surface Alignment Proof's runtime probes. The dispatcher owns file reads/writes, `code` launch, and diff display. Mirrors `record-engine.ts`'s pure/IO split.
...
- **Delegation via subprocess + JSON parse**, mirroring `browser-use/src/preflight-warm-chrome.ts`. A single `delegateWorktree(verb, args)` helper shells out, captures JSON, and surfaces upstream failures as `wt` diagnostics — extracted before the third call site (`new`/`rm`/`clean`).
- **Branch→focus guess heuristic**: strip the `type/` prefix, map a trailing `-refactor`/`-harden`/etc. token off the branch, then probe for `skills/<remainder>`; fall back to repo root if no match. Always overridable via `wt focus`. Cheap and wrong-tolerant per origin.
...
skills/wt/
  ...
  src/
    ...
    wt-engine.ts              # Engine owner: PURE render + color-assign + focus-guess + drift-compare
    wt-discovery.ts           # Discovery owner: live `git worktree list --json`, repo→workspace-path, registry load/freshness
    wt.ts                     # CLI owner: argv dispatch, file IO, code launch, diff render, diagnostics
    wt.test.ts                # Test owner: Command Surface Alignment Proof + engine unit tests
```
...
```mermaid
flowchart LR
  A[wt.config.json<br/>branch-keyed prefs] --> E[wt-engine<br/>PURE render]
  B[git worktree list --json<br/>via wt-discovery] --> E
...
```mermaid
flowchart TD
  CLI[wt dispatcher] --> SYNC[sync → engine → drift gate → write]
...
The greenfield layout (new directory hierarchy under `skills/wt/`) is shown in the HTD module-ownership tree above. Per-unit `**Files:**` are authoritative.
...
**Goal:** Stand up `skills/wt/` mirroring `record-decision`'s package shape so the contract and engine have a home.
**Requirements:** R14, R15.
**Dependencies:** none.
**Files:** `skills/wt/package.json`, `skills/wt/tsconfig.json`, `skills/wt/SKILL.md`, add `wt.config.json` + `*.code-workspace` to the repo `.gitignore`.
**Approach:** Copy `record-decision`'s `package.json`/`tsconfig.json` shape; set `bin`/script to `src/wt.ts`, dep `@side-quest/cli-command-facade: workspace:*`. SKILL.md follows the thin owner-naming pattern (Owner / Dependencies / Workflow / route to `create-cli` before contract changes) — no copied flags or schemas.
...
**Dependencies:** U2.
**Files:** `skills/wt/src/wt-engine.ts`, `skills/wt/src/wt-engine.test.ts`.
**Approach:** Pure functions, no I/O. `renderWorkspace()` builds the `folders` array (focus pair per worktree, WIP folder pinned first), `settings` (`files.exclude` + mirrored `search.exclude` + `fileNesting` + per-worktree `colorCustomizations` keyed via multi-root folder settings or `window.title`). `assignColor(branch, prefs)` = registry value or `palette[djb2(branch) % palette.length]`. `guessFocus(branch, repoRoot)` per the KTD heuristic. `stampHeader(bodyJSON)` prepends the `// GENERATED by wt … hash:<h>` comment; `computeHash(body)` + `readHeaderHash(file)` + `isDrift(file)` support R12/R13.
...
- Registry: absent `wt.config.json` → empty registry; malformed JSON → `registry_unreadable`.
...
**Dependencies:** U3, U4.
**Files:** `skills/wt/src/wt.ts`, extend `skills/wt/src/wt.test.ts`.
...
**Requirements:** R9, R10.
**Dependencies:** U5.
**Files:** extend `skills/wt/src/wt.ts`, extend `skills/wt/src/wt-discovery.ts` (or a sibling `wt-delegate.ts`), extend `skills/wt/src/wt.test.ts`.
...
**Requirements:** R14.
**Dependencies:** U5, U6.
**Files:** extend `skills/wt/src/wt.test.ts`.
**Approach:** Scenario tests using facade testing-subpath helpers where available. Cover: each advertised flag appears in rendered help; command-foreign flags excluded from help; public argv acceptance and rejection (e.g. `wt color x chartreuse` rejected exit 2; `wt sync --force` accepted); runtime semantics via probes (drift gate blocks; delegation actually calls the git CLI via the spy); result literals asserted from the package-owned constants in U2, not string copies.
...
- `wt <unknown-verb>` → usage_error exit 2 with help pointer.
...
- Multi-repo orchestration beyond one-workspace-per-repo (a `wt open` global index across all repos) — the registry is already repo-keyed, so this is additive.
- Free-form hex colors — the named palette ships first.
- Shell alias `wt`→`wt open` for ergonomics — a dotfiles change, not part of this CLI package.

---

## Risks & Dependencies

- **Upstream contract coupling (accepted):** `wt`'s delegated verbs depend on `@side-quest/git worktree`'s JSON shape and subcommand flags. If that CLI changes, U6 breaks. Mitigation: a captured fixture (U4) characterizes the shape; `delegate_failed` surfaces breakage loudly rather than silently mis-parsing.
...
- Origin requirements + locked CLI contract: `docs/brainstorms/2026-06-14-wt-worktree-workspace-renderer-requirements.md`.
