---
title: "docs: Sync create-cli result vocabulary guidance"
type: docs
status: completed
date: 2026-06-02
origin: docs/brainstorms/2026-06-02-create-cli-result-vocabulary-guidance-requirements.md
issue: 155
---

# docs: Sync create-cli result vocabulary guidance

## Summary

Update the local create-cli extension docs so they reflect the shipped PR #76 facade runtime contract and teach agents where package-owned result vocabulary belongs. Keep the change small: glossary sync, design-layer guidance, facade implementation pointer, and no new schema or example surface.

---

## Problem Frame

PR #76 hoisted several create-cli design-layer candidates into `@side-quest/cli-command-facade`: baseline exit semantics, diagnostic capability role, diagnostic trail reference, and write preview capability. The current create-cli references still contain pre-PR wording such as `contract candidate` and "declare, don't enforce" in places where the facade now validates runtime contract shape.

Browser Adapter Proof exposed a second gap. Stable package literals can drift into generic create-cli prose unless the docs name their owner. The plan updates create-cli so agents ask "who owns this literal?" before writing docs, tests, schemas, or adapters.
...
- R1. Update create-cli reference wording for the now-runtime-backed baseline exits, diagnostic capability, diagnostic trail pointer, and write preview capability.
...
- R8. Avoid package-specific literal values, full schemas, allowed-value lists, and runtime envelope examples in create-cli prose.
...
- R12. Warn that create-cli guidance must not replace honest side-effect metadata with route-name inference or generic mutation vocabulary.

**No Parallel Policy**

- R13. Leave `skills/create-cli/SKILL.md` unchanged; it already routes to the local references.
...
- **Runtime-backed wording follows PR #76 exactly.** Baseline exits, diagnostic capability role, diagnostic trail reference, and write preview capability are not candidates in create-cli guidance anymore. The docs point to facade runtime ownership for exact shape and validation.
...
- Update `CONTEXT.md`.
- Update `skills/create-cli/references/agent-native-cli-design.md`.
- Update `skills/create-cli/references/cli-command-facade.md`.
...
- Browser Adapter Proof value lists, excerpts, or file-path citations in create-cli prose.
- `skills/create-cli/SKILL.md` edits.
- `skills/create-cli/playgrounds/` refresh.
- New ADR or new create-cli reference file.
...
- **Goal:** Verify `CONTEXT.md` already names package-owned result vocabulary and reflects current runtime-backed create-cli terms.
...
- **Patterns to Follow:** Existing `CONTEXT.md` language entries; `docs/agents/domain.md`; `skills/create-cli/scripts/node_modules/@side-quest/cli-command-facade/CONTEXT.md`.
- **Test Scenarios:**
...
- **Verification:** `rg "contract candidate|Browser Adapter Proof|diagnostic_trail|route-name|sideEffects|previewExemption|dry_run|package-owned result vocabulary" skills/create-cli/references/agent-native-cli-design.md` shows no stale candidate wording, one pattern citation, no `diagnostic_trail` field-name leak, at most one `sideEffects` warning, no preview-field mapping, and intended ownership guidance.
...
- **Patterns to Follow:** Existing facade field-map style; `skills/create-cli/scripts/node_modules/@side-quest/cli-command-facade/src/command-facade.ts`; PR #76 summary; ADR 0009 bounded extension.
- **Test Scenarios:**
...
- **Verification:** `rg "does NOT judge sensible exit codes|contract candidate|capabilityRoles|diagnostic_trail|previewExemption" skills/create-cli/references/cli-command-facade.md` reflects the shipped runtime contract.
...
- **Files:**
  ...
  - `skills/create-cli/references/agent-native-cli-design.md`
  - `skills/create-cli/references/cli-command-facade.md`
  - `skills/create-cli/playgrounds/create-cli-skill.html`
  - `skills/create-cli/playgrounds/create-cli-explorer.html`
  - `CONTEXT.md`
...
- **Test Scenarios:**
  ...
  - Covers R8. No new full schemas, runtime envelopes, or allowed-value lists appear in create-cli prose.
  - Covers R16. `capabilityRoles` appears only in `cli-command-facade.md` among create-cli references.
...
- AE1. Given an agent reads create-cli guidance for exit codes, when it checks the current docs, then `"0"`, `"1"`, and `"2"` are described as facade-owned baseline meanings and extra numeric codes are package-owned.
- AE2. Given a package has stable `data.*` statuses, source labels, diagnostic labels, failure domains, runtime action ids, or extra exit codes, when create-cli guidance discusses result contracts, then it directs those literals to the package contract area rather than generic facade prose.
...
- **Stale linked package evidence:** `skills/create-cli/scripts/node_modules/@side-quest/cli-command-facade` is a local link. Mitigation: also cite PR #76 metadata and avoid relying on local line numbers in the plan.
...
- Origin: `docs/brainstorms/2026-06-02-create-cli-result-vocabulary-guidance-requirements.md`
- Glossary: `CONTEXT.md`
- Design reference: `skills/create-cli/references/agent-native-cli-design.md`
- Facade reference: `skills/create-cli/references/cli-command-facade.md`
- Skill routing: `skills/create-cli/SKILL.md`
...
- Local facade source anchors: `skills/create-cli/scripts/node_modules/@side-quest/cli-command-facade/src/command-facade.ts`, `skills/create-cli/scripts/node_modules/@side-quest/cli-command-facade/tests/command-facade.test.ts`, `skills/create-cli/scripts/node_modules/@side-quest/cli-command-facade/CONTEXT.md`
