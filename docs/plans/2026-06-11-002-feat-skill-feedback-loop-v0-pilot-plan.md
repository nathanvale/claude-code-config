...
### KTD6 — The driver invokes `record`; finished skills only signal close

A finished skill (e.g. fallow) must not call `Skill("skill-feedback", …)` itself — that violates the runbook (`skills/create-skill/references/skill-design-decision-runbook.md`: "skills do not invoke other skills"), fallow's own safety policy (`skills/fallow/references/safety.md`: "do not invoke other skills from the runner"), and the composition research ("hand back to the driver, never skill-to-skill"). v0 wiring: fallow gains a `## Close` section that *signals* completion (close = an evidence command ran, summary reported, no pending rerun / doctor-repair / apply); the **driver** then invokes `skill-feedback record`. On Codex, the native `notify=agent-turn-complete` hook is the legal trigger.
...
**Approach:** Define the v0 receipt allow-list as typed constants — `skill`, `goal` (free-text), `outcome` (`confirmed`/`failed`/`ambiguous`), `friction` (free-text), `explanation` (free-text, optional), warehouse tags (`skill_version`, `git_sha`, `model`, `usage`), plus passed-in `generated_ts`. **Reject** unknown fields (storage-routing rule — not silently drop). Define the `SoftwareLearningReport` record including `untrusted_evidence: true` (R18a), `degraded`, and `redactions` count (R21). Carry the outcome enum inside the facade envelope `data` (mirror `FALLOW_STATUS_VALUES`). Declare the `record` command via `defineCommandFacadeContract` (Contract owner). No redaction or writing here — schema + shape only. Read `skills/create-skill/references/skill-design-decision-runbook.md` before authoring (AGENTS.md hard rule).
...
**Approach:** Implement redaction as **explicit, testable patterns** over **all** free-text fields (`goal`, `friction`, `explanation`) recursively before any write (defect 1, R20a): structured-secret shapes (bearer token, JWT `eyJ…`, PEM block, DSN with inline creds) **plus** prefix-keyed cloud tokens (`ghp_`, `xoxb-`, `AKIA`, `sk-`, `glpat-`) **plus** auth-param URL stripping (`?token=…`, `#frag`, non-http schemes dropped). No undefined "entropy scan" (defect 2); if a future shape needs entropy detection, define threshold/alphabet/window in code with a test. Cite `skills/agent-reliability-guardrails/references/logging-redaction-rules.md` as the always-redact owner; reuse the recursive-redact approach from `skills/browser-use/src/browser-use-core.ts`.
...
- Manual end-to-end (`Covers R1, R3.`): the driver invokes `skill-feedback record` with a degraded receipt (skill+goal+outcome only) at a real fallow close; one degraded record lands in `.skill-feedback/` with missing fields marked and the untrusted marker present.
- Close signal present: fallow's `## Close` names the close condition and the three exclusion states; it does **not** instruct fallow to invoke another skill (`Covers KTD6 / R2.`).
...
- **Stop hook** (Claude) end-of-run capture — the reliable Claude-side trigger. Codex already has `notify=agent-turn-complete`; the explicit driver call is the v0 Claude stand-in.
- Wire a **rougher / newer skill** as a second emitter if the fallow-only corpus is too thin to evaluate (fallow is mature → low friction; see Risks).
...
- **Capture still depends on the driver acting at close.** The driver invoking `skill-feedback record` is a prose instruction in the Claude path, not a harness-enforced trigger — the same reliability gap the Stop hook would close (deferred). Codex's `notify` hook is reliable; Claude's driver call is the v0 stand-in. If the Claude miss-rate is high, that is itself a valid pilot finding ("v0 needs the Stop hook").
