...
Refactor Issue-to-PR into a v2 runbook that keeps the current safety model while making the active path readable: a hot router, tiered references, deterministic `cli.ts` route facts, runtime contracts in code, and templates for repeated cross-agent handoffs. The landing shape is atomic: implementation can happen in reviewable internal units, but the public runbook should cut over only after the reference tree, CLI, ledger versioning, install topology, and regression coverage are ready together.

---

## Problem Frame

`runbooks/issue-to-pr/issue-to-pr.md` is now 1,313 lines and mixes stage routing, role contracts, ledger schema, helper command details, prompt payloads, final-review remediation, persona lookup tables, and fallback driver text. The audit concludes that the architecture is worth preserving, but the hot file has stopped behaving like a hot path.
...
- R1. Keep Issue-to-PR v2 as a runbook refactor with atomic cutover, not a formal skill migration or dual v1/v2 deprecation window.
- R2. Rewrite `issue-to-pr.md` as a 400-500 line orchestration router, with core invariants, resumed-turn routing, reference-loading triggers, stage shells, and stop conditions inline.
- R3. Move detailed policy, role contracts, lookup tables, and prompt payloads into tiered `references/` and `templates/` files with visible "read when" triggers.
...
- R11. Add `runbook_version` as a workflow-contract ledger field initialized to `2`, install the whole `runbooks/issue-to-pr/` directory recursively, and detect installed-vs-ledger version skew before continuing a run.
...
- `runbooks/issue-to-pr/issue-N-ledger.template.md` is the durable ledger seed and should gain `runbook_version` plus shorter pointers to v2 reference docs.
...
- Land v2 as an atomic runbook cutover. Intermediate commits may be reviewable, but the branch should not expose a hot runbook that references missing v2 assets or only partially adopts v2 routing.
- Keep the router in `issue-to-pr.md`, not in a separate `stage-router.md`. Resume safety is hot-path orchestration.
...
- Split helper internals first, then add the v2 CLI surface. `decompose.ts` remains a compatibility shim while `cli.ts` becomes the stable deterministic front door.
...
- Treat missing installed references, templates, `cli.ts`, or `lib/` artifacts as diagnostic drift that blocks dispatch and packet rendering until resolved.
...
The expected v2 source shape is:
...
flowchart TB
  ...
  U2 --> U7["U7 hot router cutover"]
  U5 --> U7
  U6 --> U7
  U7 --> U8["U8 README index"]
  U7 --> U9["U9 regression probes + verification"]
  U8 --> U9
```
...
**Files:**
- Create: `runbooks/issue-to-pr/references/regression-matrix.md`
...
**Approach:**
- Build a line-map from current `issue-to-pr.md`, `README.md`, `issue-N-ledger.template.md`, and `decompose.ts` command modes to their v2 destinations.
- Enumerate the audit's prose-only invariants in `references/regression-matrix.md`: Local Law Read Order, Mechanic Discipline, Public Contract Rule, Domain Language Rule, Preflight Checklist, Probe Catalog, final-review patch decision tree, smallest contract patch heuristic, mechanical-diff fallback, broad-reviewer fallback, selector precedence, and host-readiness versus infrastructure-failure boundary.
...
**Test scenarios:**
- Happy path: every current major runbook section has exactly one planned v2 destination or an explicit removal reason.
...
- `references/regression-matrix.md` names the manual and automated coverage expected before v2 ships.

### U2. Create V2 References and Templates Without Public Cutover
...
**Files:**
- Create: `runbooks/issue-to-pr/references/stage-1-pick-issue.md`
- Create: `runbooks/issue-to-pr/references/stage-2-plan.md`
- Create: `runbooks/issue-to-pr/references/stage-3-decompose.md`
- Create: `runbooks/issue-to-pr/references/stage-4-batch-loop.md`
- Create: `runbooks/issue-to-pr/references/stage-5-final-review.md`
- Create: `runbooks/issue-to-pr/references/stage-6-ship.md`
- Create: `runbooks/issue-to-pr/references/builder-dispatch.md`
- Create: `runbooks/issue-to-pr/references/findings-and-validators.md`
- Create: `runbooks/issue-to-pr/references/ledger-and-helper.md`
- Create: `runbooks/issue-to-pr/references/host-adapters.md`
- Create: `runbooks/issue-to-pr/templates/ce-plan-addendum.md`
- Create: `runbooks/issue-to-pr/templates/builder-work-packet.md`
- Create: `runbooks/issue-to-pr/templates/builder-return-envelope.md`
- Create: `runbooks/issue-to-pr/templates/validator-envelope.md`
- Create: `runbooks/issue-to-pr/templates/proposer-envelope.md`
- Create: `runbooks/issue-to-pr/templates/patch-proposal.md`
...
- Do not yet rewrite `issue-to-pr.md` to depend on these files.
...
### U3. Split Helper Internals Behind the Existing Compatibility Surface

**Goal:** Move deterministic helper logic out of the monolithic `decompose.ts` into modules without changing existing command behavior.
...
- Create: `runbooks/issue-to-pr/lib/contract.ts`
- Create: `runbooks/issue-to-pr/lib/contract.test.ts`
- Create: `runbooks/issue-to-pr/lib/digest.ts`
- Create: `runbooks/issue-to-pr/lib/digest.test.ts`
- Create: `runbooks/issue-to-pr/lib/ledger.ts`
- Create: `runbooks/issue-to-pr/lib/ledger.test.ts`
- Create: `runbooks/issue-to-pr/lib/validate.ts`
- Create: `runbooks/issue-to-pr/lib/validate.test.ts`
...
- Keep `decompose.ts` as the compatibility entrypoint for all current flags while importing the new modules.
...
**Verification:**
- Current helper regression coverage passes through `decompose.ts`.
...
**Files:**
- Create: `runbooks/issue-to-pr/cli.ts`
- Create: `runbooks/issue-to-pr/cli.test.ts`
- Create: `runbooks/issue-to-pr/lib/route.ts`
- Create: `runbooks/issue-to-pr/lib/route.test.ts`
- Modify: `runbooks/issue-to-pr/references/ledger-and-helper.md`
- Modify: `runbooks/issue-to-pr/references/regression-matrix.md`
...
- Do not remove existing `decompose.ts` flags in this unit.
...
**Files:**
- Create: `runbooks/issue-to-pr/lib/packets.ts`
- Create: `runbooks/issue-to-pr/lib/packets.test.ts`
- Modify: `runbooks/issue-to-pr/cli.ts`
- Modify: `runbooks/issue-to-pr/cli.test.ts`
- Modify: `runbooks/issue-to-pr/templates/builder-work-packet.md`
- Modify: `runbooks/issue-to-pr/templates/builder-return-envelope.md`
- Modify: `runbooks/issue-to-pr/templates/proposer-envelope.md`
- Modify: `runbooks/issue-to-pr/templates/validator-envelope.md`
- Modify: `runbooks/issue-to-pr/templates/patch-proposal.md`
- Modify: `runbooks/issue-to-pr/templates/ce-plan-addendum.md`
- Modify: `runbooks/issue-to-pr/references/builder-dispatch.md`
- Modify: `runbooks/issue-to-pr/references/findings-and-validators.md`
...
- Modify: `runbooks/issue-to-pr/lib/contract.ts`
- Modify: `runbooks/issue-to-pr/lib/ledger.ts`
- Modify: `runbooks/issue-to-pr/lib/ledger.test.ts`
- Modify: `runbooks/issue-to-pr/cli.ts`
- Modify: `runbooks/issue-to-pr/cli.test.ts`
- Modify: `runbooks/issue-to-pr/references/ledger-and-helper.md`
- Modify: `runbooks/issue-to-pr/references/host-adapters.md`
- Modify: `install.sh`
...
- Add install/status checks that verify `runbooks/issue-to-pr/references/`, `templates/`, `cli.ts`, and `lib/` are present through the installed runbook path.
...
### U7. Rewrite `issue-to-pr.md` as the V2 Hot Router

**Goal:** Replace the current policy manual hot file with a concise router that loads references and calls `cli.ts` for deterministic facts.
...
- Modify: `runbooks/issue-to-pr/references/stage-1-pick-issue.md`
- Modify: `runbooks/issue-to-pr/references/stage-2-plan.md`
- Modify: `runbooks/issue-to-pr/references/stage-3-decompose.md`
- Modify: `runbooks/issue-to-pr/references/stage-4-batch-loop.md`
- Modify: `runbooks/issue-to-pr/references/stage-5-final-review.md`
- Modify: `runbooks/issue-to-pr/references/stage-6-ship.md`
- Modify: `runbooks/issue-to-pr/references/ledger-and-helper.md`
- Modify: `runbooks/issue-to-pr/templates/ce-plan-addendum.md`
...
**Execution note:** Change-first for prose after U1-U6 have created the safety net; verify through regression matrix and probes before treating it as ready.
...
**Verification:**
- `issue-to-pr.md` is within 400-500 lines, or includes the required worked overflow enumeration.
...
### U8. Rewrite README as a Human Index

**Goal:** Remove duplicate policy from README and make it a map to the runbook, references, templates, helper, and install contract.
...
**Verification:**
- Search confirms removed policy blocks no longer exist as full duplicate manuals in README.
...
### U9. Add Hybrid Regression Probes and Final Cutover Verification
...
**Files:**
- Modify: `runbooks/issue-to-pr/references/regression-matrix.md`
- Create: `runbooks/issue-to-pr/v2-regression.test.ts`
- Modify: `runbooks/issue-to-pr/cli.test.ts`
- Modify: `runbooks/issue-to-pr/decompose.test.ts`
- Modify: `runbooks/issue-to-pr/lib/route.test.ts`
- Modify: `runbooks/issue-to-pr/lib/packets.test.ts`
- Modify: `runbooks/issue-to-pr/README.md`
...
- Run the full helper/CLI test surface after the public cutover rewrite.
...
- V2 cutover is ready for human review without implementing a live Issue-to-PR run.

---

## System-Wide Impact

- **Interaction graph:** The active workflow shifts from one large Markdown file to a hot router plus references, templates, CLI, modules, and ledger versioning. The Orchestrator still owns judgment and durable gates.
...
- **API surface parity:** `decompose.ts` compatibility flags remain during migration, while `cli.ts --json` becomes the v2 agent-facing surface.
...
| Manual prose invariants regress because tests cannot execute judgment | Maintain `references/regression-matrix.md` with explicit v1/v2 manual scenarios and residual risk notes. |
...
- The useful review order is: U1 line-map and regression matrix, U2 references/templates, U3 helper split, U4-U5 CLI and packet surfaces, U6 version/install, U7 hot router, U8 README, U9 regression proof.
...
- Focused Bun tests for `runbooks/issue-to-pr/decompose.test.ts`, `runbooks/issue-to-pr/cli.test.ts`, `runbooks/issue-to-pr/v2-regression.test.ts`, and new `runbooks/issue-to-pr/lib/*.test.ts`.
...
- `issue-to-pr.md` is a 400-500 line hot router, or includes a worked enumeration justifying any overflow.
- Every v2 reference is linked from the hot file or one active parent reference, with visible load triggers.
- README is a human index and no longer repeats detailed workflow policy.
...
- `decompose.ts` remains as a compatibility shim during migration rather than absorbing new v2 modes.
