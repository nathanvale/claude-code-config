...
The winning shape is wider for a reason. Full claim-safe `ReviewResultData` keeps runtime capture, Trusted skill identity, Daily pilot readiness, evidence tiers, allowed claims, and anchor facts in one contract-owned result. Renderers and future agents consume those facts instead of reconstructing them from partial state.
...
- R1. Extend `ReviewResultData` with `review_units`, `ledger_entries`, `anchor_miss_telemetry`, evidence tiers, entry/global `allowed_claims`, `claim_readiness`, open actions, and no-action reason.
- R2. Keep coverage, low-signal, open-item, and no-action review output ahead of ledger detail.
- R3. Expose `review_unit_key`, `ledger_anchor_key`, `anchor_strength`, and `weak_anchor_reason` on reducer-owned data.
...
- Carry facts plus allowed claims in `ReviewResultData`.
...
- Add top-level `review_units`, `ledger_entries`, `anchor_miss_telemetry`, `claim_readiness`, and `allowed_claims`.
...
- KTD7. **Readiness is split by claim.** Runtime capture, Trusted skill identity, and Daily pilot readiness can advance independently.
...
- **Claim Strategy:** Claim derivation is a contract-owned Strategy over evidence tier, source mix, trusted review-unit state, and readiness facts. Renderers do not invent claim language.
...
**Approach:** Add v2 review result types for review units, ledger entries, anchor facts, evidence tiers, allowed claims, claim readiness facts, open actions, no-action reason, and anchor-miss telemetry. Keep exact enum values and parser behavior in `command-contract.ts`. Keep docs thin and point to the contract owner.
...
- Ledger entries expose `review_unit_key`, `ledger_anchor_key`, anchor strength, source mix, weak-anchor quarantine, evidence tier, allowed claims, resolution state, verification burden, and next safe action.
- Top-level data exposes review units, ledger entries, anchor-miss telemetry, readiness by claim, global allowed claims, open actions, and no-action reason.
- `capture_readiness` is absent from v2 review output.
- A no-ledger review still returns valid v2 coverage and no-action data.

**Verification:** Contract tests prove the exported data vocabulary, contract result id, generated help references, and JSON shape agree.
...
**Approach:** Replace the current "any `skill_run_id` links" review-unit behavior with trusted-run semantics. A trusted `skill_run_id` produces a run-prefixed `review_unit_key`. Missing, untrusted, or placeholder run ids produce report-local review units.
...
**Approach:** Replace the current collapsed `capture_readiness` output with `claim_readiness`. Runtime capture can become ready from trusted hook configuration and Stop evidence. Trusted skill identity remains blocked without engine-owned identity evidence. Daily pilot remains blocked until the accepted pilot gate passes. Each readiness fact carries `status`, `reason_ids`, and `evidence_refs`.
...
- Runtime capture, Trusted skill identity, and Daily pilot readiness advance independently.
...
**Approach:** Update JSON output to expose reducer-owned ledger entries, allowed claims, anchor telemetry, and `claim_readiness`. Update plain output to keep coverage, low-signal, no-action, and open triage before ledger detail. Render claim labels from allowed claims only. Keep renderer code free of merge, corroboration, trust, and readiness inference. Do not preserve `capture_readiness` as a v2 compatibility alias.
...
**Approach:** Mark the old v2 plan as historical context. Keep Decisions 21-28 as accepted sources for claim-safe reducer scope, contract rules, golden vectors, field names, renderer migration, implementation order, and prototype absorption. Update glossary and report-shape prose to name review units, anchor Adapter facts, allowed claims, and claim readiness. Delete prototype files and package scripts only after their scenarios are represented in normal tests.
