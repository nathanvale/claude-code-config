...
Browser Use now has one Browser Authentication Transaction with reviewed-Runbook and freeform entry modes, managed and user-present access leases, approved Item Binding, disposable delivery, exact-origin and CDP-target proof, generic multi-step login, bounded attestations, Shared Run persistence, and restart revalidation. Human Identity Attestation remains part of the reviewed-run ceremony only; freeform auth never manufactures that fallback. Expired approved-submit resumes mint a fresh session attestation before one-shot dispatch.
...
### Two entry modes and 1Password access paths

Browser Use remains one product with one Browser Authentication Transaction. Tokens, service accounts, desktop sign-in, and biometrics are distinct trust levels inside that product, not separate products.

| Entry mode | Admission | Identity ceremony | Shared engine |
|---|---|---|---|
| Reviewed Runbook | Declared auth context inside the reviewed end-to-end run | May use the runbook's signed Human Identity Attestation when stable identity proof is insufficient | `skills/browser-use/src/browser-use-runbook-auth.ts` |
| Freeform browsing | Already-authorized agent presents a verified handoff, service id, and exact allowed origin through `browser-use auth login` | Never manufactures a Runbook or invokes the Runbook attestation fallback | `skills/browser-use/src/browser-use-runbook-auth.ts` |

Both modes acquire one short-lived Browser Authentication Access Lease before credential delivery:

- **Managed service authority:** the unattended security provider injects one managed, read-only, exactly-one-vault service token only into transaction-bounded retrieval work. Browser Use, the agent, the adapter, and the parent shell never receive token bytes or rely on an ambient `OP_SERVICE_ACCOUNT_TOKEN` or persistent `op` session.
- **User-present desktop authority:** when managed authority is unavailable, one bounded desktop sign-in or biometric unlock session owns retrieval for that transaction. Its provider returns the same handle-only retrieval port and releases the session at transaction end. It does not become a Runbook or a generic interactive shell fallback.
- **Unavailable authority:** when neither path exists, return `awaiting-auth` with one typed continuation before target mutation or confidential delivery.

The browser-facing session receives only an opaque credential handle/result for approved field delivery. The private transaction engine may project ID-only metadata to prove exactly-one-vault scope and reproduce an approved item binding, but exposes no public vault-list operation. It never grants item creation or update, account administration, raw token access, or general `op` authority. User-requested vault listing and item creation or update remain explicit operations in the separate `one-password` workflow and are never inherited by a Browser Use session.

Owners:

- Contract: `skills/browser-use/src/command-contract.ts`.
- Model: `skills/browser-use/src/browser-use-auth-access.ts`, `skills/browser-use/src/browser-use-run-model.ts`, and `skills/browser-use/src/browser-use-auth-model.ts`.
- Engine: `skills/browser-use/src/browser-use-auth-transaction.ts` and `skills/browser-use/src/browser-use-runbook-auth.ts`.
- Access authority: the admitted Browser Use Security seam in `skills/browser-use/src/browser-use-runtime.ts`; native product admission remains owned by `runtime/browser-use-security`.
- Discovery: Browser Connect Verified Handoff plus the current same-origin CDP target.
- CLI: `skills/browser-use/src/browser-use-parser.ts` and `skills/browser-use/src/browser-use.ts`.
