...
- Before any file edit, verify isolation. In every code repo, isolate first with the `worktree` skill (new/attach); never edit its main checkout. The configured Super-vault in `~/.config/context/vault.md` is the sole exception: make scoped vault writes directly on `main`, never create vault worktrees, preserve unrelated state, and allow only one canonical writer at a time. Read-only work stays in the main checkout.
