...
- Before any file edit, verify isolation. In every code repo, isolate first with the `worktree` skill (new/attach); never edit its main checkout. The configured Super-vault in `~/.config/context/vault.md` is the sole exception: route vault writes through the `vault-git` skill; if it reports activation blocked, make scoped vault writes directly on `main` and preserve unrelated state; never create vault worktrees; allow only one canonical writer at a time. Read-only work stays in the main checkout.
