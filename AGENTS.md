...
- Claude/Codex MCP keys: use `$HOME/code/dotfiles/bin/with-one-password-token` or an approved keychain owner; never create provider auth bins, use `with-env`, or depend on ambient shell env. For auth checks, never source `.env` or print key prefixes; check wrapper presence, `op`/keychain readiness, and MCP config. If Codex Context7 auth is missing, use `npx -y ctx7 ...` and record the gap.
