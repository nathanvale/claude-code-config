...
export const setupBranchStationCatalog = [
	...
	station({ id: "sync.vault_git_inputs_required", command: "sync", trigger: "vault-git domain needs private enrollment inputs", exit: 0, status: "ok", action: "provide_host_enrollment_inputs", mutation: readOnly }),
	station({ id: "sync.vault_git_selected", command: "sync", trigger: "vault-git domain proves one enrolled selected runtime", exit: 0, status: "ok", action: "setup_healthy", mutation: readOnly }),
	station({ id: "sync.vault_git_ssh_prerequisite", command: "sync", trigger: "private enrollment inputs fail repository SSH prerequisites", exit: 1, status: "ok", action: "provision_repository_ssh", mutation: noWrite }),
	station({ id: "sync.vault_git_enrollment_ready", command: "sync", trigger: "validated private inputs await explicit vault-git apply", exit: 1, status: "ok", action: "apply_host_enrollment", mutation: checkOnly }),
	station({ id: "sync.vault_git_applied", command: "sync", trigger: "explicit vault-git apply installs and selects one content-addressed runtime", exit: 0, status: "ok", action: "setup_healthy", mutation: writes }),
	station({ id: "sync.vault_git_noop", command: "sync", trigger: "explicit vault-git apply revalidates the existing enrollment", exit: 0, status: "ok", action: "setup_healthy", mutation: noWrite }),
	station({ id: "sync.vault_git_selection_blocked", command: "sync", trigger: "active or uncertain vault work refuses Runtime Selection after installation", exit: 1, status: "ok", action: "wait_for_vault_git_idle", mutation: writes }),
	station({ id: "sync.vault_git_rollback_ready", command: "sync", trigger: "rollback preview proves one verified prior runtime", exit: 1, status: "ok", action: "apply_vault_git_rollback", mutation: checkOnly }),
	station({ id: "sync.vault_git_rollback_applied", command: "sync", trigger: "rollback selects the verified prior runtime", exit: 0, status: "ok", action: "setup_healthy", mutation: writes }),
	station({ id: "sync.vault_git_rollback_blocked", command: "sync", trigger: "active or uncertain vault work refuses rollback", exit: 1, status: "ok", action: "wait_for_vault_git_idle", mutation: noWrite }),
