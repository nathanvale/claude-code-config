...
async function main(): Promise<number> {
	...
	const admittedEntitlements = join(scratchRoot, "ApprovalBroker.entitlements.plist");
	...
	await cp(installedApp, stagedApp, { recursive: true, preserveTimestamps: true });
	await cp(unsignedBroker, join(stagedApp, "Contents", "MacOS", "ApprovalBroker"));
	...
	const nextApp = join(productRoot, `.ApprovalBroker.next-${installationID}.app`);
...
try {
	...
	process.stderr.write(
		`${JSON.stringify({
			status: "blocked",
			code: error instanceof Error ? error.message : "install-failed",
		})}\n`,
	);
	process.exitCode = 20;
}
