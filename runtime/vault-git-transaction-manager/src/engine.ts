	...
	return {
		async begin(input) {
			...
			if (acquired.status === "refused") {
				...
				return refusal(phase === "human_required" ? "human_required" : "unknown", phase, acquired.blocker, acquired.nextAction.id, acquired.nextAction.summary, acquired.changedState);
			}
		...
		async inspect() {
			...
			const observed = await observeRemoteLedger(options.ledger, { remote: "origin" });
			...
			if (phaseState) return result("inspected", phaseState, loaded, "denied", "none", nextForState(phaseState), summaryForState(phaseState));
		...
		async recordPhase(input) {
			const loaded = await options.store.load();
			if (loaded.status !== "loaded" || loaded.receipt.transactionId !== input.transactionId) {
			...
			const fenced = await fence(loaded.receipt, input.remote);
			if (fenced) return fenced;
			...
			const recorded = nextReceipt(loaded.receipt, {
				phase: input.phase,
				transition,
				nextSafeAction: input.nextSafeAction,
				commitId: input.commitId ?? loaded.receipt.commitId,
				recordedAt: options.runtime.now().toISOString(),
			});
...
function result(status: VaultGitEngineResult["status"], state: VaultGitTransactionState, receipt: VaultGitReceipt, writePermission: VaultGitWritePermission, changedState: VaultGitEngineResult["changedState"], actionId: string, summary: string, blocker?: VaultGitBlockerId): VaultGitEngineResult {
...
function refusal(state: VaultGitTransactionState, phase: VaultGitTransactionPhase, blocker: VaultGitBlockerId, actionId: string, summary: string, changedState: VaultGitEngineResult["changedState"] = "none"): VaultGitEngineResult {
	return { status: "refused", state, phase, writePermission: "denied", changedState, retrySafety: state === "human_required" || state === "superseded" || state === "expired" ? "operator_required" : "same_input_unsafe", blocker, nextAction: { id: actionId, summary } };
}

function inspected(state: VaultGitTransactionState, phase: VaultGitTransactionPhase, actionId: string, summary: string): VaultGitEngineResult {
...
function nextForState(state: VaultGitTransactionState): string { return state === "push_pending" ? "retry_push" : state === "repairable" ? "run_repair" : state === "human_required" ? "request_operator_review" : "none"; }
