...
| --- | --- | --- |
| `fixed` | `commit <sha>` or `patch-batch patch-NNN` or `plan-revision <sha>` | The finding was fixed by a reachable Builder commit recorded in a terminal ledger batch, or a terminal patch-batch with reachable Builder commits. Fixed Stage 3 findings (`batch_id: stage-3`) use `plan-revision <sha>` where the SHA is the reachable plan/DAG revision that closed the finding. |
