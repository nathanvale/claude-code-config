...
| `pick-issue` | Ledger exists but derived AC state not yet `confirmed` (status not `confirmed`, or `ac_digest` null/mismatched) | Run [Stage 1 shell](#stage-1-shell-pick-issue) from the AC-confirmation step. |
