...
- **Profile:** `~/.cache/chrome-agent` -- shared Chrome daemon from the session registry
- **Session:** `--session {session_id}` for tab isolation
...
Read the config for `credentials`, `identity`, and `services.zoom` entries only. **Ignore** `config.chrome.*` fields -- Chrome lifecycle is managed by the session registry, not per-service configs.
